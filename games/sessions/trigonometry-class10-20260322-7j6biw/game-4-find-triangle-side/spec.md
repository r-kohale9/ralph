<!--
  ...
  MISCONCEPTION TARGETED: Students confuse the co-function relationship (cos A = cosecant A) and
    sin θ can exceed 1 (NCERT Ex 8.1 Q11: 'sin θ = 4/3 for some θ' — False). Ancheta (2022): 64%/77%.
  SOURCE: https://jett.labosfor.com/index.php/jett/article/download/1064/681/5186
  ...
  NCERT: Ch 8 §8.2 Ex 8.2 (14 questions) — standard angle values; Ex 8.1 Q11(v) sin θ = 4/3 False
  SESSION ID: trigonometry-class10-20260322-7j6biw
  GENERATED: Phase 5 — Session Planner spec generation, 2026-03-23
-->

# Find the Missing Side — Session Variant (Trig Class 10, Game 4 of 5)
...
> **PEDAGOGICAL DESIGN NOTE.** This game implements Bloom's Level 3 (Apply) for the skill `find-side`: given a right triangle with one acute angle and one known side, the learner must (1) identify which trig ratio to use, then (2) compute the unknown side. Session position: Game 4 of 5 — builds on ratio computation (Game 3), prerequisite for real-world application (Game 5). Target misconceptions: co-function confusion (cos A = cosecant A; Ancheta 2022 — 77% misconception rate) and sin θ > 1 belief (NCERT Ex 8.1 Q11(v): 'sin θ = 4/3 for some θ' is False). The two-step decomposition prevents guessing: Step 1 is MCQ (select ratio: sin / cos / tan). Step 2 is typed numeric input (PART-014, tolerance ±0.15). SVG right-triangle diagram rendered per round. Incorrect step-1 reveals worked-example before step 2 unlocks. No lives on step-1 errors — only step-2. NCERT alignment: Ch 8 §8.2 Ex 8.2 (standard angle values), Ex 8.1 Q11(v). Interaction type: `two-step-ratio-plus-typed`.
...
- **Type:** standard
- **Description:** Students identify the correct trig ratio (sin/cos/tan) for a given angle and known side, then compute the missing side using typed numeric input. 5 rounds with distinct triangle configurations. Two-step interaction: ratio selection (MCQ) followed by computation (typed). Targets Grade 10 / NCERT Ch 8 §8.2 Ex 8.2 / CC HSG-SRT.C.8 / Bloom L3 Apply. Addresses misconceptions: co-function confusion (cos = cosecant) and sin θ > 1. Session position: Game 4 of 5. Prerequisite: soh-cah-toa-worked-example (Game 3). Successor: real-world-problem (Game 5).
