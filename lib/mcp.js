...
- "skip review" → call with skip_review: true`,
  ...
  server.tool(
    ...
    {
      game_session_id: z.string().describe('Game session ID (play_id) to fetch events for'),
    },
    async ({ game_session_id }) => {
      try {
        const { Storage } = require('@google-cloud/storage');
        const storage = new Storage();
        const bucket = storage.bucket('test-dynamic-assets');

        // 1. Fetch session + signal summary from core API
        const sessionData = await coreApiFetch(`/api/sessions?game_id=&limit=200`).catch(() => ({ data: { sessions: [] } }));
        const session = (sessionData.data?.sessions || []).find((s) => s.id === game_session_id);

        if (!session) {
          return { content: [{ type: 'text', text: `No session found with id: ${game_session_id}` }], isError: true };
        }

        // 2. Build GCS prefix from session data
        const filesPath = `signal-events/${session.userId || 'unknown'}/${session.sessionId || 'unknown'}/${session.gameId}/${session.contentSetId || 'unknown'}/${game_session_id}/`;

        // 3. List and download all batch files
        const [files] = await bucket.getFiles({ prefix: filesPath });
        const batches = [];
        ...
        // 4. Fetch game metadata (includes game URL)
        const gameData = await coreApiFetch(`/api/games/${session.gameId}`).catch(() => null);

        // 5. Fetch game HTML if URL available
        let gameHtml = null;
