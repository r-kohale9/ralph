      ...
      return {
        content: [{
          ...
          text: [
            ...
            '6. SHOW the complete spec to the user and ask for their approval before submitting',
            '7. Only after user approves, call register_spec to submit it for building',
          ].join('\n'),
        }],
      };
    },
...
IMPORTANT: Do NOT call this tool automatically. ONLY call it when the user EXPLICITLY says "register", "register game", "register spec", or similar. Never call it as part of spec generation — generating a spec and registering are two completely separate actions.

When generating a spec (user says "create a game spec" or similar):
1. Use get_warehouse_guide, read parts/rules/contracts, generate the spec
2. Create the spec as an artifact/document titled "[Game Title] — Spec v1"
3. In chat, say: "I've created the spec document — please review it and let me know if you'd like to register."
4. STOP and WAIT for the user to explicitly say "register". Do NOT call register_spec.

When the user says "register" (or "register game", "register spec"):
1. Call this tool with the spec content. It will return a review prompt for you to execute.
2. Execute the review. If 0 Critical/Warning findings → call register_spec again with skip_review: true.
3. If there ARE Critical/Warning findings → show them to the user and STOP.

After review findings are shown, the user may respond:
- "fix these N issues" → fix only those issues IN THE SAME spec artifact (never create a new one), WAIT for user to say "register"
- "fix all" → fix all issues in the same artifact, call register_spec again (without skip_review) to re-review. Repeat up to 5 cycles max. Then ask user to confirm.
- "fix all and register" → same as "fix all" but register automatically after all issues resolved
- "skip review" or "register anyway" → call this tool with skip_review: true

NEVER create a new spec artifact/document. Always update the existing one in place.`,
    {
    ...
    async ({ game_id, title, description, spec_content, skip_review }) => {
      if (!skip_review) {
        ...
        if (!reviewPrompt) {
          return {
            content: [{ type: 'text', text: 'SPEC-REVIEW-PROMPT.md not found in warehouse. Cannot review spec.' }],
            isError: true,
          };
        }
        return {
          content: [{
            type: 'text',
            text: [
              'REVIEW REQUIRED BEFORE REGISTRATION. You MUST now EXECUTE the review process below against the provided spec.\n',
              'Follow every step, produce findings in the exact format specified.\n',
              'Create an artifact/document titled "[Game Title] — Review Findings" with ALL detailed findings.\n',
              'In chat, show ONLY a summary: verdict + count by severity.\n\n',
              'If 0 Critical/Warning findings → call register_spec again with skip_review: true to complete registration.\n',
              'If there ARE Critical/Warning findings → show them to the user and STOP. Do NOT register.\n',
              'Info-level findings do not block registration.\n\n',
              'After presenting findings, the user may respond:\n',
              '- "fix these N issues" → fix only those issues IN THE SAME spec artifact (never create a new one), WAIT for user to say "register"\n',
              '- "fix all" → fix all issues, re-run register_spec (without skip_review) to re-review. Repeat up to 5 cycles max. Then ask user to confirm.\n',
              '- "fix all and register" → same loop but register automatically after all issues resolved\n',
              '- "skip review" or "register anyway" → call register_spec with skip_review: true\n\n',
              '---\n\n',
              reviewPrompt,
              '\n---\n\n# Spec to Review\n\n',
              spec_content,
