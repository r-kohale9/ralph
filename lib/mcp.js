      ...
      return {
        content: [{
          ...
          text: [
            ...
            'OUTPUT RULES (strict):\n',
            '1. Execute the full review and compile ALL findings with full detail (severity, category, what spec says, problem, suggested fix)\n',
            '2. You MUST create an artifact titled "[Game Title] — Review Findings" with the COMPLETE findings formatted as markdown. This is the primary deliverable.\n',
            '3. AFTER the artifact, write a short chat summary (under 10 lines): verdict + one-line per finding\n',
            '4. Do NOT duplicate the full findings in chat text — the artifact has the details\n\n',
            'NEXT STEPS:\n',
            `- 0 Critical/Warning findings → call register_spec with game_id="${game_id}", skip_review=true, and the spec_content to complete registration\n`,
            '- Has Critical/Warning findings → show summary and STOP. Do NOT call register_spec. Do NOT register.\n',
            '- Info-level findings do not block registration.\n\n',
            'After presenting findings, the user may respond:\n',
            '- "fix N issues" → fix in the SAME spec artifact (never create a new one), WAIT for user to say "register"\n',
            '- "fix all" → fix all issues in the same artifact, call register_spec again (without skip_review) to re-review. Max 5 cycles.\n',
