...
function createMcpServer(deps = {}) {
  ...
  server.tool(
    'register_spec',
    'Register a game spec for building. Creates/updates the game entry and queues a build. CRITICAL: You MUST show the complete generated spec to the user and get their explicit approval BEFORE calling this tool. Never submit a spec without the user reviewing it first. The workflow is: (1) call get_warehouse_guide, (2) read needed parts/rules/contracts, (3) generate the spec, (4) SHOW the spec to the user and ask for approval, (5) only after user says yes, call this tool.',
    {
  ...
  server.tool(
    'review_spec',
    'Review a game spec for concept correctness, interaction flaws, broken promises, and completeness. Pass the full spec content. Returns structured findings. Call this BEFORE register_spec to catch issues early.',
    {
      spec_content: z.string().describe('The full game spec markdown to review'),
    },
    async ({ spec_content }) => {
