...
function validatePostMessageContract(html) {
  ...
  if (hasGameComplete) {
    ...
    if (hasDataMetrics) {
      ...
      // Preview screen fields (if PreviewScreenComponent is used)
      const hasPreviewScreen = /PreviewScreenComponent|previewScreen\s*\.\s*show/.test(html);
      if (hasPreviewScreen) {
        if (!/duration_data[\s\S]{0,200}preview/.test(html)) {
          errors.push(
            'CONTRACT: PreviewScreenComponent detected but duration_data.preview not found — ' +
              'duration_data should include preview: [] array tracking preview screen timing'
          );
        }

        // v2: .game-wrapper visibility must NOT be manually toggled — preview wrapper owns layout
        if (/\.game-wrapper[\s\S]{0,80}style[\s\S]{0,40}display\s*=/.test(html) ||
            /querySelector\(['"]\.game-wrapper['"]\)[\s\S]{0,80}display/.test(html)) {
          errors.push(
            'CONTRACT: .game-wrapper display is manually toggled, but PreviewScreenComponent v2 ' +
              'creates the persistent wrapper — .game-wrapper does not exist when previewScreen:true. ' +
              'Remove all .game-wrapper display toggles.'
          );
        }
      }
...
function validateSignalCollectorContract(html) {
  ...
  // ── 3. All 6 signalCollector properties must be set from signalConfig ─────
  // PART-010: handlePostMessage must configure these 6 properties on signalCollector
  // when game_init arrives with signalConfig. Missing any means data won't route correctly.
  const requiredProps = ['flushUrl', 'playId', 'gameId', 'sessionId', 'contentSetId', 'studentId'];
  const missingProps = requiredProps.filter(
    (prop) => !new RegExp(`signalCollector\\.${prop}\\s*=`).test(html)
  );
  if (missingProps.length > 0) {
    errors.push(
      `CONTRACT: handlePostMessage missing signalCollector property assignments: ${missingProps.join(', ')}. ` +
        'All 6 SignalCollector properties must be configured from signalConfig in the game_init handler: ' +
        'flushUrl, playId, gameId, sessionId, contentSetId, studentId. ' +
        'See PART-010 "Batch Flushing" section for the correct pattern.'
    );
  }

  // ── 4. signalConfig must be extracted from event data ───────────────────────
  // Accept either gameState.signalConfig = ... or var/const/let signalConfig = ...data.signalConfig
  const hasSignalConfigExtract =
    /gameState\.signalConfig\s*=/.test(html) ||
    /(?:var|let|const)\s+signalConfig\s*=\s*.*signalConfig/.test(html);
  if (!hasSignalConfigExtract) {
    errors.push(
      'CONTRACT: signalConfig is never extracted from game_init data — ' +
        'handlePostMessage must store signalConfig: gameState.signalConfig = d.signalConfig || {}; ' +
        'See PART-010 "Batch Flushing" section.'
    );
  }
...
// ─── Video player contract (PART-040) ───────────────────────────────────────

function validateVideoContract(html) {
  const errors = [];

  // Only validate if HTML contains a <video> element
  if (!/<video\b/.test(html)) return errors;

  // REQUIRED: controls attribute
  if (!/<video[^>]*\bcontrols\b/.test(html)) {
    errors.push('VIDEO [PART-040]: <video> missing `controls` — user cannot play the video');
  }

  // REQUIRED: playsinline attribute
  if (!/<video[^>]*\bplaysinline\b/.test(html)) {
    errors.push('VIDEO [PART-040]: <video> missing `playsinline` — iOS will hijack to fullscreen');
  }

  // FORBIDDEN: autoplay
  if (/<video[^>]*\bautoplay\b/.test(html)) {
    errors.push('VIDEO [PART-040]: <video> has `autoplay` — blocked by browsers, causes silent failure');
  }

  // FORBIDDEN: loop (games need to detect video end)
  if (/<video[^>]*\bloop\b/.test(html)) {
    errors.push('VIDEO [PART-040]: <video> has `loop` — game must detect ended event for flow control');
  }

  return errors;
}

// ─── Audio player contract (PART-041) ───────────────────────────────────────

function validateAudioPlayerContract(html) {
  const errors = [];

  // Only validate if HTML contains an <audio> element
  if (!/<audio\b/.test(html)) return errors;
  ...
  // FORBIDDEN: new Audio() — must use DOM element
  if (/new\s+Audio\s*\(/.test(html)) {
    errors.push('AUDIO [RULE-006]: new Audio() found — use <audio> element in DOM instead');
  }
...
function validateContract(html) {
  ...
  errors.push(...validateVideoContract(html));
  errors.push(...validateAudioPlayerContract(html));
