  ...
  for (const [key, metric] of Object.entries(histograms)) {
    ...
    if (metric.values.length > 0) {
      ...
      const labelStr = Object.entries(metric.labels)
        ...
        .map(([k, v]) => `${k}="${v}"`)
        .join(',');
      const quantileBase = labelStr ? `${metric.name}{${labelStr},` : `${metric.name}{`;
...
const _sysGauges = {
  cpu: { name: 'ralph_cpu_usage_percent', help: 'CPU usage percentage (1-second sample)', value: 0 },
  ramUsed: { name: 'ralph_ram_used_bytes', help: 'RAM used bytes', value: 0 },
  ramPct: { name: 'ralph_ram_usage_percent', help: 'RAM usage percentage', value: 0 },
  disk: { name: 'ralph_disk_usage_percent', help: 'Disk usage percentage (data directory)', value: 0 },
};

function _setSysGauge(key, value) {
  _sysGauges[key].value = value;
  setGauge(_sysGauges[key].name, {}, value);
}
...
async function updateSystemMetrics() {
  try {
    ...
    _setSysGauge('cpu', Math.round(cpu * 10) / 10);
    _setSysGauge('ramUsed', usedMem);
    _setSysGauge('ramPct', Math.round((usedMem / totalMem) * 1000) / 10);
    try {
      const { execSync } = require('child_process');
      const df = execSync("df -k . | tail -1 | awk '{print $5}'", { encoding: 'utf8' }).trim().replace('%', '');
      _setSysGauge('disk', parseFloat(df) || 0);
    } catch (_) {}
  } catch (e) {
    // non-fatal
  }
...
function startSystemMetrics(intervalMs = 30000) {
  updateSystemMetrics();
...
async function getSystemStats() {
  ...
  return {
    ...
    ramPct: ((totalMem - freeMem) / totalMem) * 100,
  };
}
