...
// ─── 5f3. TimerComponent — NOT in CDN bundle ─────────────────────────────────
// TimerComponent is not exported by packages/components/index.js. Any reference
// to it (new TimerComponent(...) or TimerComponent.something) causes
// "ReferenceError: TimerComponent is not defined" at runtime, crashing
// DOMContentLoaded before ScreenLayout.inject() runs → blank page.
if (/\bTimerComponent\b/.test(html)) {
  errors.push(
    'FORBIDDEN: TimerComponent is not exported by the CDN components bundle — it throws ReferenceError at runtime and crashes the game init. Use setInterval/setTimeout for timers instead.'
  );
}
