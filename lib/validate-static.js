...
// ─── Per-game spec context + opt-outs ───────────────────────────────────────
// Sources read once at validator startup:
//   1. `alfred/overrides/floating-button-opt-outs.md` (central allowlist) —
//      if the current game ID is listed, FloatingButton rules are auto-skipped.
//   2. `games/<id>/spec.md` (sibling of the HTML) — surfaces numeric spec
//      fields (totalRounds, totalLives) + spec flags (retryPreservesInput) so
//      validator rules can condition on them.
//
// Why the allowlist lives outside the game dir: an earlier version put the
// opt-out inside `games/<id>/spec.md` as `floatingButton: false` and was
// exploited — build sub-agents (step 4) wrote the flag into the spec themselves
// using the exact "failed reasoning" patterns the validator error warned
// against, to silence the rule. The allowlist lives in the `alfred/overrides/`
// tree which the sub-agent has no natural reason to edit during step 4 (its
// expected output is `games/<id>/index.html`). Any diff to the allowlist from
// a sub-agent run is a visible scope violation the user can revert.
//
// Numeric spec fields (totalRounds, totalLives, retryPreservesInput) ARE read
// from spec.md because they are legitimate per-game design parameters that the
// plan author writes during step 1/3 — they are not opt-outs. If the
// sub-agent fabricates these values during step 4 to silence a rule, the
// resulting plan→HTML inconsistency will be visible in the generated game's
// gameState init and in the plan doc.
const specContext = (function readSpecContext() {
  const out = {
    floatingButton: null,          // false => allowlisted, skip FB rules
    totalRounds: null,             // integer, from spec.md
    totalLives: null,              // integer, from spec.md
    retryPreservesInput: null      // boolean, from spec.md
  };
  ...
  try {
    const allowlistPath = path.resolve(
      ...
      'alfred',
      'overrides',
      'floating-button-opt-outs.md'
    );
    if (fs.existsSync(allowlistPath)) {
      const gameId = path.basename(path.dirname(path.resolve(htmlPath)));
      ...
      if (entries.includes(gameId)) {
        out.floatingButton = false;
      }
    }
  } catch (e) { /* fall through; rules default to strict */ }

  // ── 2. Sibling spec.md numeric / boolean fields ─────────────────────────
  try {
    const specPath = path.join(path.dirname(htmlPath), 'spec.md');
    if (fs.existsSync(specPath)) {
      const spec = fs.readFileSync(specPath, 'utf-8');

      // totalRounds: match forms like `**Rounds:** 1`, `Rounds: 1`, `totalRounds: 1`.
      // Prefer the first explicit numeric occurrence after a "Rounds" label.
  ...
  } catch (e) { /* spec unreadable — leave fields as null */ }

  return out;
})();

// Back-compat alias — earlier code referenced `specOptOuts`. Keep it pointing
// at the same object so the existing FB-rules block continues to work.
...
(function checkFloatingButtonLifecycle() {
  ...
  while ((smm = setModeNextRe.exec(html)) !== null) {
    ...
    const hasSeparator = /transitionScreen\.(hide|onDismiss)\s*\(|\bawait\b|\.then\s*\(/.test(window);
  ...
  if (tooEarlyOffsets.length > 0) {
    ...
    errors.push(
      'ERROR [GEN-FLOATING-BUTTON-NEXT-TIMING]: `floatingBtn.setMode(\'next\')` is called too early — ' +
        ...
        '`transitionScreen.onDismiss(...)` / `await` / `.then()` separating them. Call site line(s): ' + lineNumbers + '. ' +
