...
const endGameMatch = html.match(/(?:async\s+)?function\s+endGame\s*\([^)]*\)\s*\{([\s\S]*?)(?=\nfunction |\n(?:const|let|var)\s+\w|\n\/\/\s*─|$)/);
if (endGameMatch) {
  const endGameBody = endGameMatch[1];
  const endGameDecl = endGameMatch[0];
  const bodyHasAwait = /\bawait\b/.test(endGameBody);
  const declIsAsync = /^async\s+function/.test(endGameDecl.trim());
  if (bodyHasAwait && !declIsAsync) {
    errors.push('MISSING: endGame() uses await but is not declared async — must be async function endGame()');
  }
}
