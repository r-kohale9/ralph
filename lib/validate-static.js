...
{
  // Find static feedback div declarations: id or class containing 'feedback'
  const feedbackDivRe = /<div[^>]+(?:id|class)\s*=\s*["'][^"']*feedback[^"']*["'][^>]*>/gi;
  ...
  if (withoutAriaLive.length > 0) {
    warnings.push(
      `WARNING [ARIA-001]: ${withoutAriaLive.length} feedback div(s) found without aria-live attribute — ` +
      'WCAG SC 4.1.3 requires dynamic feedback regions to have aria-live="polite" or aria-live="assertive" ' +
      'so screen readers announce feedback without requiring focus change. ' +
      'Apply to: #correct-feedback, #feedback, .feedback-message, and any div whose id/class contains "feedback". ' +
      `Missing on: ${withoutAriaLive.slice(0, 3).map((el) => el.slice(0, 60)).join(' | ')}`,
    );
  }
}
