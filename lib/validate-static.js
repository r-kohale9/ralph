...
// Root cause of disappearing-numbers #442 failure (Lesson 98).
if (/\bTimerComponent\b/.test(html)) {
  ...
  if (/new\s+TimerComponent\s*\(\s*\{/.test(html) || /new\s+TimerComponent\s*\(\s*document\./.test(html)) {
    warnings.push(
      'WARNING: TimerComponent called with wrong first argument. ' +
        ...
        "First arg must be the element ID string, not a DOM element or options object (Lesson 98)."
    );
  }
}
