...
// refs at init instead. (stats-mean-direct #575 root cause. Lesson L-GF-005.)
{
  const perRoundFunctionPattern =
    /(?:function\s+(?:loadQuestion|loadRound|showRound|showQuestion|renderOptions)\s*\([^)]*\)|(?:loadQuestion|loadRound|showRound|showQuestion|renderOptions)\s*=\s*(?:function|\([^)]*\)\s*=>))\s*\{[^}]*document\.(getElementById|querySelector)\s*\(/;
  if (perRoundFunctionPattern.test(html)) {
    warnings.push(
      'WARNING [GEN-DOM-CACHE]: document.getElementById() or document.querySelector() called inside a per-round function ' +
        '(loadQuestion/loadRound/showRound/showQuestion/renderOptions). ' +
