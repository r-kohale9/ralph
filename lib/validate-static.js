...
// syncDOMState() reads window.gameState.phase and sets data-phase on document.body
// (GEN-SYNC-TARGET: must target body, not #app — LP-4). If a phase assignment
// happens without a nearby syncDOMState() call, data-phase is never updated
// → ALL waitForPhase() calls timeout.
...
// ─── GEN-SYNC-TARGET: syncDOMState() must target document.body not #app ──────
// LP-4 (2026-03-23): Confirmed in count-and-tap #551 and real-world-problem #564.
// The generated game's own syncDOMState() sets data-phase/lives/round on
// document.getElementById('app') or document.querySelector('#app') instead of
// document.body. Contract tests that assert body[data-phase] or body[data-lives]
// always return null when the game writes to #app instead of body.
// REQUIRED: document.body.dataset.phase = gameState.phase (not appEl.dataset.phase)
// HEURISTIC: detect getElementById('app').dataset.phase or querySelector('#app').dataset.phase
// anywhere in the HTML (they almost always appear inside the game's syncDOMState()).
{
  const appDatasetPhase = /getElementById\s*\(\s*['"]app['"]\s*\)\s*\.dataset\s*\.\s*phase/.test(html);
  const appQueryDatasetPhase = /querySelector\s*\(\s*['"]#app['"]\s*\)\s*\.dataset\s*\.\s*phase/.test(html);
  const appVarDatasetPhase = /\bapp(?:El|Element)?\b\s*(?:&&\s*)?\.\s*dataset\s*\.\s*phase\s*=/.test(html);
  if (appDatasetPhase || appQueryDatasetPhase || appVarDatasetPhase) {
    warnings.push(
      'WARNING [GEN-SYNC-TARGET]: syncDOMState() is setting data-phase on #app element instead of document.body. ' +
      'Contract tests assert on body[data-phase], body[data-lives], body[data-round] — writing to #app makes ALL these assertions return null. ' +
      'WRONG: document.getElementById(\'app\').dataset.phase = gameState.phase; ' +
      'WRONG: appEl.dataset.phase = gameState.phase; ' +
      'RIGHT: document.body.dataset.phase = gameState.phase; ' +
      'WHY: Tests assert on body[data-phase], body[data-lives], body[data-round]. Setting attributes on #app makes all contract/game-flow phase assertions return null. ' +
      '(LP-4, count-and-tap #551, real-world-problem #564)'
    );
  }
}
