...
10. CDN COMPONENT CLEANUP — do NOT destroy in endGame():
    - NEVER call progressBar.destroy(), timer.destroy(), or visibilityTracker.destroy() in endGame()
    - Destroying them removes CDN DOM elements (#mathai-progress-slot, #mathai-timer-slot) from the page
    - Tests check these elements AFTER game over — destroying them causes "element not found" failures
    - Safe cleanup: let them be garbage collected on page unload, or delay cleanup by 10+ seconds:
...
- NEVER call progressBar.destroy(), timer.destroy(), or visibilityTracker.destroy() in endGame() — destroys CDN DOM elements (#mathai-progress-slot, #mathai-timer-slot); tests check them AFTER game over
