...
- **NEVER use page.evaluate() to read game state data for assertions** — this is fragile. Use DOM assertions (toHaveText, toContainText, toBeVisible) on actual UI elements instead. If you MUST check game state: correct form is \`expect(await page.evaluate(() => window.x)).toBe(v)\` — NOT \`await expect(page.evaluate(() => window.x)).toBe(v)\` (the latter passes a Promise, not a value).
...
- **Slot IDs and their content**:
  - \`#mathai-transition-slot\` — contains the transition screen with a **button** and title. Use for: transition button clicks (\`#mathai-transition-slot button\`), transition title (\`.mathai-transition-title\`). Do NOT check rounds or lives here.
  - \`#mathai-progress-slot\` — contains the **progress bar**: rounds display (\`.mathai-rounds-display\`) and lives display (\`.mathai-lives-display\`). Use for rounds/lives only. NO buttons here.
  - NEVER look for buttons in \`#mathai-progress-slot\`. NEVER look for rounds/lives in \`#mathai-transition-slot\`.
- **ID vs class**: Both are ELEMENT IDs — always use \`#\` prefix, never \`.\` (dot). Also: \`.mathai-rounds-display\` and \`.mathai-lives-display\` are CSS classes, use \`.\` for those.
...
${htmlContent}`;
  ...
  // Fix common LLM mistake: looking for buttons in #mathai-progress-slot (progress bar — no buttons!)
  // Buttons are in #mathai-transition-slot. Only fix button-finding patterns, not the slot itself.
  tests = tests.replace(/(locator\s*\(['"]#mathai-progress-slot['"]\))\s*\.locator\s*\(\s*['"]button['"]/g,
    "locator('#mathai-transition-slot').locator('button'");
  tests = tests.replace(/locator\s*\(\s*['"]#mathai-progress-slot\s+button['"]/g,
  ...
  // Fix common LLM mistake: await expect(page.evaluate(() => ...)).toBe(value)
  // page.evaluate() returns a Promise — it must be awaited BEFORE passing to expect()
  // Wrong: await expect(page.evaluate(() => window.x)).toBe(v)
  // Right: expect(await page.evaluate(() => window.x)).toBe(v)
  tests = tests.replace(
    /await\s+expect\s*\(\s*page\.evaluate\s*\(/g,
  ...
  await page.waitForFunction(
    ...
    if (tests.includes('#mathai-progress-slot button') || tests.includes("'#mathai-progress-slot').locator('button'")) {
      tests = tests.replace(/locator\s*\(\s*['"]#mathai-progress-slot\s+button['"]/g, "locator('#mathai-transition-slot button'");
      tests = tests.replace(/(locator\s*\(['"]#mathai-progress-slot['"]\))\s*\.locator\s*\(\s*['"]button['"]/g,
        "locator('#mathai-transition-slot').locator('button'");
      changed = true;
    }
