...
- NEVER assert exact CDN ProgressBar lives text like toHaveText("❤️❤️") — use page.evaluate(() => window.gameState.lives) instead
- NEVER assert exact timer display text — use page.evaluate to check gameState or just verify timer is visible
- To trigger endGame without playing all rounds: page.evaluate(() => { window.gameState.currentRound = window.gameState.totalRounds; window.endGame('victory'); })
