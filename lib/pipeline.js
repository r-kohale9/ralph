...
async function captureGameDomSnapshot(gameDir, transitionSlotId, logger) {
  ...
  try {
    ...
    // Format: show visible elements with id, tag, classes, and short text
    const fmt = (items) =>
      items
        .filter((e) => e.id && e.visible)
        .map((e) => {
          const cls = e.classes ? ` (classes: ${e.classes})` : '';
          const txt = e.text ? ` — "${e.text}"` : '';
          return `  #${e.id} [${e.tag}]${cls}${txt}`;
        })
        .join('\n') || '  (none visible)';

    const snapshot = {
      startScreen: startDom.filter((e) => e.id && e.visible),
      gameScreen: gameDom.filter((e) => e.id && e.visible),
