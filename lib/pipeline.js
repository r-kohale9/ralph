...
async function runPipeline(gameDir, specPath, options = {}) {
  ...
  const genPrompt = `You are an expert HTML game developer. Generate a complete, single-file HTML game based on the following specification.
...
${specContent}`;
  ...
  try {
    ...
    llmCalls.push({ step: 'generate-html', model: GEN_MODEL });
  } catch (err) {
...
Fix ALL the listed structural issues. Output the complete corrected HTML wrapped in a \`\`\`html code block.`;
  ...
  const testPrompt = `You are an expert Playwright test writer. Generate a comprehensive Playwright test suite for the following HTML game.

Write tests covering these 9 MANDATORY categories:
...
${htmlContent}`;
  ...
  try {
    serverProc = require('child_process').spawn(
      'npx', ['-y', 'serve', gameDir, '-l', '8787', '-s', '--no-clipboard'],
    ...
    for (let iteration = 1; iteration <= MAX_ITERATIONS; iteration++) {
      ...
      try {
        const { stdout } = await execFileAsync('npx', [
          'playwright', 'test',
          '--config', path.join(gameDir, 'playwright.config.js'),
          '--reporter=json',
...
module.exports = { runPipeline, extractHtml, extractTests };
