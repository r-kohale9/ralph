...
${specContent}`;
  ...
  // Extract the transition slot ID so the LLM knows the right readiness check
...
CRITICAL — GAME INITIALIZATION (read carefully):
This game loads external packages from CDN (FeedbackManager, TimerComponent, etc.) before initializing.
\`window.gameState\` is set at script parse time — it is defined BEFORE the packages load or the UI renders.
Waiting for \`window.gameState !== undefined\` is WRONG — it resolves immediately before anything is ready.

CORRECT beforeEach pattern:
\`\`\`javascript
test.beforeEach(async ({ page }) => {
  await page.goto('/');
  // Wait for CDN packages to load and the start screen button to appear in the DOM
...
- After clicking the start/transition button, add \`await page.waitForTimeout(500)\` before the next interaction to allow CSS animations to complete
- When the same button text ("Let's go!") appears on multiple screens, click with a wait between each
