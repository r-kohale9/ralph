...
function getRelevantLearnings(gameId, limit = 10) {
  try {
    ...
    const lines = rows.map((r) => `- [${r.category}] ${r.content}`).join('\n');
    return lines;
  } catch {
...
module.exports = { runPipeline, runTargetedFix, extractHtml, extractTests, extractSpecMetadata, injectTestHarness, validateSpec, extractTestGenerationHints, getRelevantLearnings };
