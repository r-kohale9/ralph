  ...
  try {
    ...
    if (specContent) {
      ...
      if (specKeywords.size > 0) {
        const specKeywordStr = Array.from(specKeywords).join(' ');
        kept.sort((a, b) => {
          const scoreA = jaccardSimilarity(a.content, specKeywordStr);
          const scoreB = jaccardSimilarity(b.content, specKeywordStr);
          return scoreB - scoreA; // descending: most relevant first
        });
      }
    }
