...
${specContent}`;
  ...
  if (!smokeResult.ok) {
    ...
    const smokeErrorContext = [
      ...
      `Fix this error. The game MUST include <div id="gameContent"></div> in the body — CDN games render their UI inside this element. Do NOT remove or rename this element.`,
      `Ensure CDN packages load correctly using the exact script URLs from the spec.`,
    ].join('\n');
