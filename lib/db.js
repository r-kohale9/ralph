...
function getDb() {
  ...
  db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_session_games_plan_id ON session_games(plan_id);
  `);
...
// ─── Session Planner CRUD ─────────────────────────────────────────────────────

function createSession(planId, concept, games, requestedBy) {
  const insertSession = getDb().prepare(
    'INSERT INTO sessions (plan_id, concept, game_sequence, requested_by) VALUES (?, ?, ?, ?)',
  );
  const insertGame = getDb().prepare('INSERT INTO session_games (plan_id, game_id, position) VALUES (?, ?, ?)');
  const run = getDb().transaction(() => {
    insertSession.run(planId, concept, JSON.stringify(games.map((g) => g.gameId)), requestedBy || null);
    for (const g of games) { insertGame.run(planId, g.gameId, g.position); }
  });
  run();
  return planId;
}
...
function getSession(planId) {
  const session = getDb().prepare('SELECT * FROM sessions WHERE plan_id = ?').get(planId);
  if (!session) return null;
  const games = getDb().prepare('SELECT * FROM session_games WHERE plan_id = ? ORDER BY position ASC').all(planId);
  return { ...session, games };
}

function updateSession(planId, fields) {
  const setClauses = [];
  const params = [];
  if (fields.status !== undefined) { setClauses.push('status = ?'); params.push(fields.status); }
  if (fields.current_game_index !== undefined) { setClauses.push('current_game_index = ?'); params.push(fields.current_game_index); }
  if (setClauses.length === 0) return;
  params.push(planId);
  getDb().prepare(`UPDATE sessions SET ${setClauses.join(', ')} WHERE plan_id = ?`).run(...params);
}

function updateSessionGame(planId, gameId, fields) {
  const setClauses = [];
  const params = [];
  if (fields.buildId !== undefined) { setClauses.push('build_id = ?'); params.push(fields.buildId); }
  if (fields.status !== undefined) { setClauses.push('status = ?'); params.push(fields.status); }
  if (setClauses.length === 0) return;
  params.push(planId, gameId);
  getDb().prepare(`UPDATE session_games SET ${setClauses.join(', ')} WHERE plan_id = ? AND game_id = ?`).run(...params);
}
