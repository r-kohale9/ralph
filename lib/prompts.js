...
- GEN-SVG-CONTRAST: SVG stroke and fill colors MUST meet WCAG AA contrast ratio (≥4.5:1 against background). NEVER use: stroke="#64748b" (borderline fail at small sizes), fill="#94a3b8" (too light), fill="#6b7280" (fails on white). ALWAYS use: stroke="#374151" (9.3:1 on white), stroke="#1f2937" (14:1 on white), or stroke="currentColor" (inherits accessible text color). (find-triangle-side #549 UI-FTS-007, which-ratio #561 — 2 confirmed instances)
...
48. GEN-SVG-CONTRAST (SVG STROKE/FILL COLORS — WCAG AA contrast required):
    SVG stroke and fill colors MUST meet WCAG AA contrast ratio (≥4.5:1 against background).
    NEVER use: stroke="#64748b" (contrast ~4.5:1 — borderline fail at small sizes),
               fill="#94a3b8" (too light on white), fill="#6b7280" (fails on white)
    ...
    WRONG:
      ...
      <circle fill="#94a3b8" .../>          ← too light, fails WCAG AA
      <line stroke="#6b7280" .../>          ← fails on white background

    RIGHT:
      <path stroke="#374151" .../>          ← 9.3:1 contrast on white — passes WCAG AA
      <path stroke="#1f2937" .../>          ← 14:1 contrast on white — exceeds WCAG AA
      <path stroke="currentColor" .../>     ← inherits accessible text color from CSS

    WHY: SVG lines and shapes at small sizes need high-contrast colors to remain visible for users
    with low vision. #64748b fails at text-equivalent sizes per WCAG 1.4.3 Non-Text Contrast.
