...
- PREVIEWSCREEN (PART-039, MANDATORY): PreviewScreenComponent MUST be instantiated AFTER ScreenLayout.inject(). Constructor: new PreviewScreenComponent({ autoInject: true, slotId: 'mathai-preview-slot', gameContentId: 'gameContent' }). In game_init handler, call previewScreen.show({ questionLabel, score, instruction, audioUrl, onComplete: startGameAfterPreview }). gameState.startTime MUST be set in startGameAfterPreview(), NOT in setupGame() or DOMContentLoaded. VisibilityTracker onInactive → previewScreen.pause(), onResume → previewScreen.resume().
...
2. CDN INIT ORDER — immutable sequence:
   ...
   // In game_init handler: call previewScreen.show({ questionLabel, score, showStar, instruction, audioUrl, previewContent, onComplete: startGameAfterPreview })
