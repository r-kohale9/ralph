...
- GEN-SIGNAL-RESET: restartGame() MUST call signalCollector.reset() BEFORE calling showStartScreen() — endGame() seals the collector; if not reset, all analytics in session 2+ are silently lost
...
50. **GEN-SIGNAL-RESET: restartGame() must reset SignalCollector**
    WRONG: function restartGame() { gameState.currentRound = 0; /* ... */ showStartScreen(); }  // collector still sealed
    RIGHT: function restartGame() { signalCollector.reset(); gameState.currentRound = 0; /* ... */ showStartScreen(); }
    WHY: endGame() calls signalCollector.seal() to finalize the postMessage payload. A sealed collector silently ignores all subsequent recordViewEvent()/startProblem() calls. Confirmed: real-world-problem #564 restartGame() — console shows "[WARNING] [SignalCollector] Sealed — cannot record" for all round 2+ events.
...
- GEN-SIGNAL-RESET: restartGame() must call signalCollector.reset() before showStartScreen() (endGame seals it — sealed collector silently drops all round 2+ signals)
- GEN-MCQ-PHASE (ALL GAMES — MANDATORY data-phase state machine): EVERY Ralph pipeline game MUST declare data-phase transitions. This is NOT optional for any game type — drag-and-drop, canvas, worked-example, text-input, and MCQ games all require it. Each function that changes game phase MUST set gameState.phase AND call syncDOMState() immediately after — in this exact order:
  showStartScreen()  → gameState.phase = 'start';      syncDOMState();  // before transitionScreen.show()
  startGame()        → gameState.phase = 'playing';    syncDOMState();  // after transitionScreen.hide(), before renderRound()
  renderRound()      → gameState.phase = 'playing';    syncDOMState();  // at top of function, before rendering options
