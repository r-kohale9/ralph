...
- CRITICAL CDN NAMESPACE RULE — CDN components are BARE WINDOW GLOBALS. NONE of these namespaces exist:
  ...
  VALID bare globals (assigned directly from window or used as typeof checks):
    ...
    window.SentryHelper               — part of helpers bundle (PART-012 only)
...
29. RULE-SENTRY-ORDER: initSentry() MUST be called INSIDE the waitForPackages() callback, AFTER packages load:
    SentryHelper is part of the CDN helpers bundle — it is undefined until CDN packages finish loading.
    Calling initSentry() before waitForPackages() resolves throws ReferenceError (SentryHelper undefined)
    and crashes the init block, leaving #gameContent never created.

    BAD (initSentry before waitForPackages resolves — crashes at runtime):
    \`\`\`js
    window.addEventListener('DOMContentLoaded', async () => {
      initSentry();               // ← WRONG: SentryHelper not loaded yet → ReferenceError
      await waitForPackages();
      ScreenLayout.inject(...);
    });
    \`\`\`
    ...
    window.addEventListener('DOMContentLoaded', async () => {
      await waitForPackages();
      initSentry();               // ← CORRECT: CDN helpers bundle (including SentryHelper) is now loaded
      ScreenLayout.inject(...);
    });
    \`\`\`
