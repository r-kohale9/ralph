...
**BUG 6 — progressBar.init() method does not exist (ONLY if error mentions "progressBar.init is not a function")**
ProgressBarComponent has exactly 3 API points: constructor(slotId, config), .update(currentRound, totalRounds), and .destroy(). There is NO .init(), .start(), .reset(), or any other method. If the fatal error is "progressBar.init is not a function", the bug is calling a non-existent method on ProgressBarComponent.
\`\`\`js
// WRONG — .init() does not exist:
progressBar = new ProgressBarComponent('mathai-progress-bar-slot', config);
progressBar.init();  // TypeError: not a function
...
\`\`\`
Find ALL calls to progressBar.init(), progressBar.start(), progressBar.reset(), progressBar.setLives() or any other undefined method and remove or replace them. The ONLY valid call after construction is progressBar.update(currentRound, totalRounds).
