...
- GEN-PROGRESSBAR-COUNTS-ATTEMPTED (DEFAULT POLICY): The progress bar's first arg counts ROUNDS ATTEMPTED, NOT rounds correct. \`gameState.progress\` increments on EVERY round (correct OR wrong) — by the final round of an N-round game the bar reads N/N regardless of how many rounds the student got wrong. The bump fires AFTER feedback resolves, JUST BEFORE the round-change UI (next round / Victory / Game Over) — NOT at submit. Sequence: (1) submit + state mutations (score / lives), (2) await feedback, (3) bump progress + update bar, (4) round-change UI. WRONG (counts only correct): \`if (verdict.correct) { gameState.progress++; progressBar.update(gameState.progress, lives); } else { progressBar.update(gameState.progress, lives - 1); }\` — leaves progress static on wrong answers, 10-round game with 2 wrong shows "Round 8/10" on the last round. WRONG (bumps at submit, before feedback): \`gameState.progress++; progressBar.update(...); await FeedbackManager.sound.play(...);\` — bar jumps to next round while feedback for previous round still plays. WRONG (bumps after round-change UI): \`endGame('victory'); progressBar.update(...);\` — Victory already showing with pre-bump value. RIGHT: \`if (verdict.correct) gameState.score++; else gameState.lives--; await FeedbackManager.sound.play(...); gameState.progress++; progressBar.update(gameState.progress, Math.max(0, gameState.lives)); if (gameState.lives === 0) return endGame('game_over'); if (gameState.progress >= gameState.totalRounds) return endGame('victory'); nextRound();\` — feedback first, bump after feedback / before round change. Two counters, two update sites: \`progress\` for the bar, \`score\` for the header (\`previewScreen.setScore(gameState.score + '/' + totalRounds)\`). Alternative policies (rounds-correct, points-earned, section-progress) only with explicit spec opt-in in the spec's \`## Flow\` section — never invent. See PART-023 + flow-implementation.md § Round loop pattern.
...
- GEN-ROUNDSETS-MIN-3: The fallbackContent \`rounds\` array MUST contain rounds for at least 3 distinct \`set\` values tagged \`set: 'A'\`, \`set: 'B'\`, \`set: 'C'\`. Each set contains exactly \`totalRounds\` rounds. All \`id\` values globally unique across sets — use prefix convention \`'A_r1_…'\`, \`'B_r1_…'\`, \`'C_r1_…'\`. Parallel difficulty progression across sets (Set A Round 1 difficulty ≈ Set B Round 1 ≈ Set C Round 1). The runtime \`getRounds()\` helper groups rounds by \`set\` key and rotates \`setIndex\` on each \`restartGame()\` call, so the student sees a different set on each in-session retry.
  RIGHT:
    const fallbackContent = {
      totalRounds: 2,
      rounds: [
        { id: 'A_r1_easy', set: 'A', question: '...', answer: 1 },
        { id: 'A_r2_easy', set: 'A', question: '...', answer: 2 },
        { id: 'B_r1_easy', set: 'B', question: '...', answer: 3 },
        { id: 'B_r2_easy', set: 'B', question: '...', answer: 4 },
        { id: 'C_r1_easy', set: 'C', question: '...', answer: 5 },
        { id: 'C_r2_easy', set: 'C', question: '...', answer: 6 },
      ],
    };
  WRONG: flat single-set rounds array OR only 2 distinct set values OR ids duplicated across sets OR some rounds tagged with \`set\` while others are not (mixed mode). Validator rule GEN-ROUNDSETS-MIN-3 blocks these at build time.
- GEN-STARS-COLLECTED-NO-HIDE (Stars Collected screen — exception to no-button auto-dismiss): The generic no-button auto-dismiss pattern (\`await sound.play(...); transitionScreen.hide();\` inside \`onMounted\`) applies to \`roundIntro\` and similar intermediate transition screens. It does NOT apply to the "Yay! Stars collected!" screen. For Stars Collected, \`onMounted\` MUST:
    1. \`await FeedbackManager.sound.play('sound_stars_collected', { sticker: STICKER_CELEBRATE });\`
    2. fire \`window.postMessage({ type: 'show_star', data: { count, variant: 'yellow', score: gameState.score + '/' + gameState.totalRounds } }, '*');\`
    3. \`setTimeout(function() { floatingBtn.setMode('next'); }, 1100);\`
  DO NOT call \`transitionScreen.hide()\` anywhere inside the Stars Collected \`onMounted\`. The FloatingButton's \`on('next', ...)\` handler owns the eventual cleanup: \`transitionScreen.hide(); previewScreen.destroy(); window.parent.postMessage({ type: 'game_exit' }, '*');\`. This keeps the celebration screen visible while the star animation plays into the header and the Next button appears below — instead of firing the animation against an empty background.
  WRONG:
    transitionScreen.show({ title: "Yay!\\nStars collected!", buttons: [], onMounted: async () => {
      await FeedbackManager.sound.play('sound_stars_collected', { sticker: STICKER_CELEBRATE });
      transitionScreen.hide();  // ← BUG: screen vanishes before show_star + Next button render
    }});
  RIGHT:
    transitionScreen.show({ title: "Yay!\\nStars collected!", buttons: [], onMounted: async () => {
      await FeedbackManager.sound.play('sound_stars_collected', { sticker: STICKER_CELEBRATE });
      ...
      // DO NOT call transitionScreen.hide() here — Next handler owns the hide.
    }});
