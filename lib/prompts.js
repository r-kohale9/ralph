...
30. GEN ARIA-001 (FEEDBACK ACCESSIBILITY — WCAG SC 4.1.3): ALL dynamic feedback elements MUST have aria-live="polite" AND role="status". This rule applies to ANY element whose text content changes in response to user interaction — the rule is about behavior, not element naming. Apply to: answer feedback panels, correct/incorrect indicators, score updates, hint text, phase announcements, worked-example feedback, and any div whose content or visibility changes based on game state. Confirmed audit variants that must all carry aria-live="polite": #feedback-panel, #faded-feedback, #practice-feedback, #feedback-area, #answer-feedback, #result-feedback, #feedback, .feedback-message, #correct-feedback, #incorrect-feedback, #skip-note, #hint-text. The rule applies regardless of element id or class name — if it updates dynamically, it gets aria-live="polite" and role="status".
    ...
    (Note: aria-live="assertive" is ONLY correct for a div with \`role="alert"\` AND id or class containing "error" — NEVER use assertive for correct/incorrect answer feedback. Any other use of assertive is a bug.)
...
M16. CHOICE-BTN TOUCH TARGET (MCQ/button games) — If the DOM snapshot contains '.choice-btn' elements, the mechanics test file MUST include an assertion that these buttons meet the 44px minimum touch target height (Apple HIG / GEN-TOUCH-TARGET). Dynamically generated buttons cannot be caught by static T1 checks. Pattern:
    ...
    // Alternative using CSS assertion: await expect(btn).toHaveCSS('min-height', /^([4-9][4-9]|[5-9][0-9]|\d{3,})px$/, { timeout: 2000 });
    Only include this test when '.choice-btn' appears in the actual runtime DOM snapshot. Skip if the game uses '.option-btn', '.answer-btn', or custom selectors — those have their own touch-target rules via GEN-TOUCH-TARGET.
    (UI/UX audit #513 F5d: choice-btn height unverified by any Playwright test — static T1 cannot catch dynamically generated button sizes)` : ''}
${category === 'game-flow' ? (() => {
