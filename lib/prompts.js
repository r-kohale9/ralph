...
GF8. WAITFORPHASE BEFORE EVERY toBeVisible() — MANDATORY RULE, NO EXCEPTIONS.
NEVER call toBeVisible() (or toBeHidden()) on ANY element without first calling waitForPhase() to confirm the game is in the correct phase. This applies universally — before gameplay elements, before results-screen elements, before any screen transition.
WHY: toBeVisible() checks the DOM immediately. Game elements are phase-gated — an answer-input is visible in phase=playing but NOT in phase=start. A results-screen is visible in phase=results but NOT during playing. Without waitForPhase(), the DOM may be in the WRONG phase and the assertion fails or produces a false positive. After any state-changing action (startGame, endGame, restart, skipToEnd), phase transitions involve async delays of 500ms-2s — asserting visibility without waitForPhase() is always a race condition.

WRONG (10-17 GF8 violations per build come from exactly these patterns):
    // Missing waitForPhase before playing-phase element
    await startGame(page);
    await expect(page.locator('[data-testid="answer-input"]')).toBeVisible(); // GF8 violation — game may still be in 'start' phase

    // Missing waitForPhase before results-screen element
    await skipToEnd(page, 'victory');
    await expect(page.locator('#game-screen')).toBeHidden({ timeout: 5000 }); // GF8 violation — phase not yet settled

    // Missing waitForPhase before any content assertion
    await startGame(page);
    await expect(page.locator('#question-text')).toBeVisible(); // GF8 violation — no waitForPhase call on this line or above

RIGHT (always add waitForPhase before toBeVisible/toBeHidden):
    // Playing-phase elements: waitForPhase('playing') FIRST
    await startGame(page);
    await waitForPhase(page, 'playing', 15000);
    await expect(page.locator('[data-testid="answer-input"]')).toBeVisible({ timeout: 10000 });

    // Results-screen elements: waitForPhase('results') FIRST
    await skipToEnd(page, 'victory');
    await waitForPhase(page, 'results', 20000);
    await expect(page.locator('#game-screen')).toBeHidden({ timeout: 10000 });

    // Any content element after startGame: waitForPhase('playing') FIRST
    await startGame(page);
    await waitForPhase(page, 'playing', 15000);
    await expect(page.locator('#question-text')).toBeVisible({ timeout: 10000 });

CHECKLIST — before every toBeVisible() / toBeHidden() call, verify:
    [ ] waitForPhase() appears on the line above (or earlier in the same test)
    [ ] The phase string matches the phase where the element is ACTUALLY visible
    [ ] Timeout on toBeVisible() is at least { timeout: 10000 } (CDN rendering delay)
    (identify-pairs-list #515, two-player-race #506, 10-17 GF8 violations per build — lint rule fires on EVERY toBeVisible without a preceding waitForPhase)
