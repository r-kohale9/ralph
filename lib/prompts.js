...
- GEN-SYNC-TARGET (LP-4 — syncDOMState() MUST target document.body NOT #app): The game's syncDOMState() function MUST set data-phase, data-lives, and data-round on document.body — NEVER on document.getElementById('app') or any #app variable. Contract tests assert on body[data-phase], body[data-lives], body[data-round]. Writing attributes to #app makes ALL these assertions return null and fail silently.
  WRONG: document.getElementById('app').dataset.phase = gameState.phase;
  WRONG: const appEl = document.getElementById('app'); appEl.dataset.phase = gameState.phase;
  WRONG: document.querySelector('#app').dataset.phase = gameState.phase;
  RIGHT: document.body.dataset.phase = gameState.phase;
  RIGHT: document.body.dataset.lives = String(gameState.lives);
  RIGHT: document.body.dataset.round = String(gameState.currentRound);
  WHY: Tests assert on body[data-phase], body[data-lives], body[data-round]. Setting attributes on #app makes all contract/game-flow phase assertions return null. (LP-4, count-and-tap #551, real-world-problem #564)
- GEN-MCQ-PHASE (ALL GAMES — MANDATORY data-phase state machine): EVERY Ralph pipeline game MUST declare data-phase transitions. This is NOT optional for any game type — drag-and-drop, canvas, worked-example, text-input, and MCQ games all require it. Each function that changes game phase MUST set gameState.phase AND call syncDOMState() immediately after — in this exact order:
  ...
  WHY: syncDOMState() writes gameState.phase to body[data-phase] (see GEN-SYNC-TARGET — MUST use document.body, not #app). waitForPhase() in tests polls data-phase — if syncDOMState() is not called immediately after the assignment, waitForPhase() times out and 100% of game-flow tests fail. (Root cause: 22% game-flow failures across ALL game types including drag-and-drop math-cross-grid F4 — first confirmed non-MCQ instance)
...
33. GEN-MCQ-PHASE (ALL GAMES — MANDATORY explicit data-phase state machine with syncDOMState()):
    ...
    RIGHT — syncDOMState() called immediately after assignment:
      gameState.phase = 'playing';
      syncDOMState();    // ← data-phase='playing' written to body immediately (see GEN-SYNC-TARGET)
      renderOptions();

    WHY: syncDOMState() writes gameState.phase to body[data-phase] (GEN-SYNC-TARGET: MUST target document.body, NOT #app). The test harness waitForPhase()
