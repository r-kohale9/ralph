...
| 5.8.4 | previewScreen.show() receives correct params | Inspect source for the `previewScreen.show(...)` call | `instruction` and `audioUrl` are passed from content; if spec declares a timer, `timerConfig` and `timerInstance` are also passed. Drifted names (`previewContent`, `autoInject`, `gameContentId`) must NOT appear |
