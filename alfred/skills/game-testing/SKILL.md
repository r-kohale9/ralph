...
| 1.5.5 | Standalone fallback does not silently boot with missing components | `browser_evaluate(() => window.gameState.isActive)` after the fallback timer fires (30s — see `code-patterns.md` § Standalone fallback "Fallback delay"). | If fallback fired with any required class undefined, the visible error UI is rendered and `gameState.isActive === false`. The fallback MUST NOT call `setupGame()` with a partial component graph (see `code-patterns.md` § Standalone fallback). |
