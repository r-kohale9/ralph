...
| **Iteration-1 pass rate analysis + gen prompt improvement** | **active** | Measure % of builds that pass all tests on first iteration vs needing fix loops. Identify top failing categories at iteration 1. Find gen prompt gaps. Hypothesis: adding 2-3 targeted rules for the top iteration-1 failure patterns will raise first-pass rate from ~X% to ~Y%. Analysis underway in docs/rnd-iteration1-passrate.md. | Expected: reduce avg iterations per build; fewer fix loop LLM calls per approved build |
| **Unlimited-lives / non-standard lifecycle test gen** | **next** | Live data: associations and rapid-challenge both killed for "wrong tests — lives on unlimited-lives game" (builds scored 0 useful iterations). Test-gen prompt has no signal to skip lives-based tests when `totalLives=0`. Hypothesis: injecting a spec-derived `gameFeatures` block (unlimited_lives, timed_rounds, etc.) into the test-gen prompt eliminates these kill-inducing test mismatches. Measure: zero "known unresolvable" kills across next 5 builds of variant-lifecycle games. | Eliminates a class of builds killed at 0 iterations — pure waste with no learning extracted |
