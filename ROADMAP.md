...
| **P8: Investigate pre-pipeline failures** | **active** | 5+ recent builds fail with null error_message and 0 iterations — pipeline never started. Likely BullMQ job-level errors or worker crash during job pick-up. Need: error capture at job start, better fail message, auto-requeue logic. | These builds waste queue slots silently; pattern in: position-maximizer, mcq-multi-select, keep-track, disappearing-numbers |
