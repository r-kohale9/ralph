...
**Last updated:** March 20, 2026 (rendering failure prevention R1-R5 rules shipped commit 1645449; Worker SyntaxError crash-loop resolved + 37 builds manually recovered; queue-sync orphan detection R&D started; 545 tests pass; R&D: queue-sync active)
...
| **Queue-sync: detect orphaned DB-queued builds with no BullMQ job** | **active** | Hypothesis: `requeueQueueSyncBuilds()` only rescues builds with `error_message LIKE '%queue-sync%'`, but today's crash-loop (worker SyntaxError) left 37 builds with `status='queued'` in DB but zero BullMQ jobs. Gap: no code detects `queued` DB records with no corresponding BullMQ job. Fix: add a startup scan that cross-checks `status='queued'` DB records against BullMQ queue depth — if DB shows N queued but BullMQ has 0, re-enqueue them. Expected impact: prevents silent queue loss on any future crash-loop; ~5-10 builds recovered per incident instead of requiring manual intervention. |
