...
**Last updated:** March 20, 2026 (spec FeedbackManager.init() mass fix — 48/50 specs corrected; contract Rule 5 postMessage fix; T1 W3/W4 checks; BullMQ lock renewal R&D active)
...
| **BullMQ lock renewal for long Opus calls** | **active** | make-x (#304) showing `could not renew lock for job N` every 5 minutes — Opus fix calls take 3-4 min each, exceeding BullMQ's default lock duration (30s). Worker processes the job successfully but Redis considers it stale. Risk: job gets declared failed mid-LLM call and requeued, wasting a full pipeline run. Fix: increase `lockDuration` in worker.js BullMQ Worker config (e.g., 300000ms = 5 min) and/or set `lockRenewTime` to half lock duration. Hypothesis: eliminates lock-loss failures entirely at cost of slightly slower orphan detection. |
